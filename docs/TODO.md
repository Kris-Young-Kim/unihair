...
4. **전환 이벤트로 표시**
   ...
   - `booking_form_submit_success` 이벤트 찾기
   - 해당 이벤트의 토글 스위치를 "켜기"로 변경
   - 또는 이벤트 이름 옆의 "전환으로 표시" 체크박스 선택

5. **확인**
   - 이벤트 목록에서 `booking_form_submit_success` 옆에 "전환" 배지가 표시되는지 확인
   - "보고서" → "전환" 메뉴에서 예약 완료 전환 확인 가능
...
3. **퍼널 분석 설정:**
   ...
   **방법 1: 탐색 분석 사용**
   ...
   - "탐색" → "퍼널 탐색" 선택
   - 단계별 이벤트 추가:
     1. `booking_button_click` (예약 버튼 클릭)
     2. `booking_modal_open` (모달 열기)
     3. `booking_form_submit` (폼 제출 시도)
     4. `booking_form_submit_success` (예약 완료)
   - 각 단계별 이탈률 확인 가능
...
- [ ] 예약 완료 전환 목표 설정 (수동 작업)
  - [x] 가이드 작성 완료
  - [ ] GA4 대시보드에서 이벤트 기반 전환 설정 (사용자 작업)
  - [ ] `booking_form_submit_success` 이벤트를 전환으로 표시 (사용자 작업)
- [ ] 사용자 여정 분석 설정 (선택사항)
  - [x] 가이드 작성 완료
  - [ ] 이벤트 시퀀스 분석 설정 (사용자 작업)
  - [ ] 퍼널 분석 설정 (사용자 작업)
